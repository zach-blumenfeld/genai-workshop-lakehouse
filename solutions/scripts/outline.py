...
WIRE = """
MATCH (root) WHERE ($root IS NULL AND root:Library)
   OR ($root IS NOT NULL AND root.uri = $root)
MATCH path = (root)-[:HAS*0..%d]->(n)
WITH path, n, nodes(path) AS chain
RETURN length(path) AS depth,
       CASE WHEN n:Library THEN 'Library' WHEN n:Folder THEN 'Folder'
