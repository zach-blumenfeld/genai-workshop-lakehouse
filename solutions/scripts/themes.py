...
word, the shared link targets the members converge on, the most-linked member
documents as outline rows, and crossover documents into other themes. The tool
never names a theme; it ships the evidence and the agent names it.

Reasoning (domain-agnostic): the library has no Part/DTC entity nodes - documents
are tied together purely by their cross-reference links (LINKS_TO). Two documents
land in the same theme because they link to each other or converge on the same
targets. Pipeline: project a doc-level graph (Section links collapsed to owning
documents, undirected, weighted) -> Leiden mutate themeId -> per-community
conductance (cohesion) -> write themeId -> renderer queries -> drop projection.
themeId is producer-owned and regenerated per run; theme numbers are stable only
within a run - store URIs, never T<id>.
"""
